...
// Assign a web component (html) tag name to our custom component since Leaflet
// doesn't provide it's own (it's not a Web Component)
@Tag("leaflet-map")

// Download the Leaflet JS files using NPM
@NpmPackage(value = "leaflet", version = "1.6.0")

// Include the necessary theme files from the Leaflet package and our own
// additions
@CssImport(value = "leaflet/dist/leaflet.css")
@CssImport(value = "./src/leaflet.css")

// Finally import our connector JS file (the '.' refers to the 'frontend' folder
// in the project root)
@JsModule("./src/leaflet-map.js")
public class LeafletMap extends PolymerTemplate<TemplateModel> implements HasSize
{
	private static final long serialVersionUID = -7927948576442530890L;
	/**
	 * OpenStreetMap's API links
	 */
	 ...
	 */
	private Map<Integer, Measurement> idToMarker = new HashMap<>();
	...
	public LeafletMap()
	{
		...
		addListener(InternalMarkerClickEvent.class, e ->
		{
			...
			Measurement measurement = e.getId() == null ? null : idToMarker.get(e.getId());

			// fire real event
			fireEvent(new MarkerClickEvent(this, true, measurement));
		});
	}
	 ...
	 * visible.
	 */
	public void addMarkersAndZoom(List<Measurement> measurements)
	{
		measurements.forEach(this::addMarker);
		List<MapLocation> spots = measurements.stream().map(Measurement::getMapLocation).collect(Collectors.toList());
		// Add all
	 ...
	 */
	public void addMarker(Measurement measurement)
	{
		// save id for later use in events
		idToMarker.put(nextMarkerId, measurement);
		MapLocation spot = measurement.getMapLocation();
		// call client side to actually add marker
		getElement().callJsFunction("addMarker", spot.getLatitude(), spot.getLongitude(), spot.getName(),
				measurement.getReason().getMeasurementType().name(), nextMarkerId++);
	}
	...
	public static class MarkerClickEvent extends ComponentEvent<LeafletMap>
	{
		...
		private final Measurement marker;

		public MarkerClickEvent(LeafletMap source, boolean fromClient, Measurement marker)
		{
			super(source, fromClient);
			this.marker = marker;
		}

		public Measurement getMarker()
		{
			return marker;
		}
	}
